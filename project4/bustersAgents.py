...
class GreedyBustersAgent(BustersAgent):
  ...
  def chooseAction(self, gameState):
    ...
    for distribution in livingGhostPositionDistributions:
      ...
      mostLikelyDis=self.distancer.getDistance(mostLikelyPos, pacmanPosition)
      if mostLikelyDis<dis:
        dis=mostLikelyDis
        closetGhostPos=mostLikelyPos

    action=None
    pacmanDis=float('inf')
    for act in legal:
      successorPos=Actions.getSuccessor(pacmanPosition,act)
      closetDis=self.distancer.getDistance(successorPos,closetGhostPos)
      if closetDis<pacmanDis:
        pacmanDis=closetDis
        action=act
    return action