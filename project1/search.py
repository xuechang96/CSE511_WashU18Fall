...
class SearchProblem:
    ...
    def getSuccessors(self, state):
        ...
        successor to the current state (current node), 'action' is the action
        required to get there (all path to get the node), and 'stepCost' is the incremental
    ...
    while True:
        ...
        if not currentNode in visited:
            ...
            for successor in problem.getSuccessors(currentNode):
                path = list(currentPath)
                path.append(successor[1])
                queue.push((successor[0], path), problem.getCostOfActions(path))
...
def aStarSearch(problem, heuristic=nullHeuristic):
    ...
    queue = util.PriorityQueue()
    startState = (problem.getStartState(), [])
    g_n = problem.getCostOfActions(startState[1])
    h_n = heuristic(problem.getStartState(), problem)
    f_n = g_n + h_n
    queue.push(startState, f_n)
    visited = []
    ...
    while True:
        ...
        if not currentNode in visited:
            ...
            for successor in problem.getSuccessors(currentNode):
                path = list(currentPath)
                path.append(successor[1])
                queue.push((successor[0], path), problem.getCostOfActions(path)+heuristic(successor[0], problem))
